...
int main()
{
	int i;
	u_char buf[40] = {
		0x3b, 0x36 ,0x32 ,0x31 ,0x37 ,0x30 ,0x30 ,0x31 ,0x33 ,0x37 ,
		0x30 ,0x30 ,0x32 ,0x35 ,0x36 ,0x35 ,0x38 ,0x38 ,0x37 ,0x33 ,
		0x3d ,0x32 ,0x34 ,0x30 ,0x38 ,0x32 ,0x32 ,0x30 ,0x35 ,0x31 ,
		0x31 ,0x31 ,0x30 ,0x32 ,0x30 ,0x30 ,0x30 ,0x30 ,0x3f ,0x36 };

	u_char digest[20];
	SHA1_CTX ctx;
	SHA1Init(&ctx);
	SHA1Update(&ctx, (const u_char*)buf, sizeof(buf));
	SHA1Final(digest, &ctx);
	printf("test, sha1 = \n");
	for (i = 0; i < 20; i ++)
		printf("%02x ", digest[i]);
		// e5 01 eb a6 22 c0 16 57 b5 bb 26 df 64 2f a2 78 dc 3e ae 21
	return 0;
}
#endif
