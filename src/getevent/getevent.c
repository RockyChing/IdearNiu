...
/*
 * This is `getevent` command, do not code according to self-defined APIs, such as sys_debug()
 * With this demo we know the manipulation of input device under Linux
 */
...
int close_device(const char *device, int print_flags)
{
	int i;
	for (i = 1; i < nfds; i ++) {
		if (!strcmp(device_names[i], device)) {
			int count = nfds - i - 1;
			if (print_flags & PRINT_DEVICE)
...
int getevent_test_entry(int argc, char *argv[])
{
	int c;
	int i;
	int res;
	...
	do {
		...
		if (c == EOF)
			break;
		switch (c) {
		case 't': /* show time stamps */
			get_time = 1;
			break;
		case 'n': /* dont print new line */
			newline = "";
			break;
		case 's': /* print switch states for given bits */
			get_switch = strtoul(optarg, NULL, 0);
			if (dont_block == -1)
				dont_block = 1;
			break;
		case 'S': /* print all switch states */
			get_switch = ~0;
			if (dont_block == -1)
				dont_block = 1;
			break;
		case 'v': /* verbosity mask (errs=1, dev=2, name=4, info=8, vers=16, pos. events=32, props=64) */
			if (optarg)
            ...
            print_flags_set = 1;
			break;
		case 'd': /* show HID descriptor, if available */
			print_flags |= PRINT_HID_DESCRIPTOR;
			break;
		case 'p': /* show possible events (errs, dev, name, pos. events) */
            ...
            if(dont_block == -1)
                dont_block = 1;
			break;
		case 'i': /* show all device info and possible events */
			print_flags |= PRINT_ALL_INFO;
            print_flags_set = 1;
            if(dont_block == -1)
                dont_block = 1;
			break;
		case 'l': /* label event types and names in plain text */
			print_flags |= PRINT_LABELS;
			break;
		case 'q': /* quiet (clear verbosity mask) */
			print_flags_set = 1;
			break;
		case 'c': /* print given number of events then exit */
			event_count = atoi(optarg);
			dont_block = 0;
			break;
		case 'r': /* print rate events are received */
			sync_rate = 1;
			break;
		case '?': /* */
		...
		for (i = 1; i < nfds; i ++) {
			if (ufds[i].revents) {
				if (ufds[i].revents && POLLIN) {
					...
					if (res < (int) sizeof(event)) {
						fprintf(stderr, "count not get event\n");
						return 1;
					}
