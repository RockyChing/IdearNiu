...
ssl_init (void)
{
...
#if OPENSSL_VERSION_NUMBER >= 0x00907000
  printf("+++ OPENSSL_VERSION_NUMBER >= 0x00907000 +++\n");
  ...
  if (ciphers_string && !SSL_CTX_set_cipher_list(ssl_ctx, ciphers_string))
    {
      logprintf(LOG_NOTQUIET, ("OpenSSL: Invalid cipher list: %s\n"), ciphers_string);
      goto error;
    }
...
# define FD_TO_SOCKET(X) (X)
  ...
  if (RAND_status () != 1)
    {
      logprintf(LOG_NOTQUIET,
		("WARNING: Could not seed PRNG. Consider using --random-file.\n"));
      goto error;
    }
...
ssl_check_certificate (int fd, const char *host)
{
  ...
  IF_DEBUG
    {
      ...
      logprintf (LOG_NOTQUIET, "certificate:\n  subject: %s\n  issuer:  %s\n", subject, issuer);
      xfree (subject);
      xfree (issuer);
    }
  ...
  if (vresult != X509_V_OK)
    {
      char *issuer = _get_rfc2253_formatted (X509_get_issuer_name (cert));
      logprintf (LOG_NOTQUIET, "%s: cannot verify %s's certificate, issued by %s:\n",
                 severity, host, issuer);
      xfree(issuer);
      ...
      switch (vresult)
        ...
        case X509_V_ERR_CERT_NOT_YET_VALID:
          logprintf (LOG_NOTQUIET, ("  Issued certificate not yet valid.\n"));
          break;
        case X509_V_ERR_CERT_HAS_EXPIRED:
          logprintf (LOG_NOTQUIET, ("  Issued certificate has expired.\n"));
          break;
        default:
  ...
  if (alt_name_checked == false)
    {
      ...
      if (!pattern_match (common_name, host))
        {
          logprintf (LOG_NOTQUIET, "%s: certificate common name %s doesn't match requested host name %s.\n",
    ...
    if (!pinsuccess)
      {
        logprintf (LOG_ALWAYS, "The public key does not match pinned public key!\n");
        success = false;
      }


  if (success)
    logprintf (LOG_ALWAYS, "X509 certificate successfully verified and matches host %s\n", host);
  X509_free (cert);

 no_cert:
  if (opt.check_cert == CHECK_CERT_ON && !success)
    logprintf (LOG_NOTQUIET, "To connect to %s insecurely, use `--no-check-certificate'.\n", host);
