...
void updateTableNamesFile() {
    ...
    //me comments if (!WriteStringToFile(contents, RT_TABLES_PATH, RT_TABLES_MODE, AID_SYSTEM, AID_WIFI)) {
...
int padInterfaceName(const char* input, char* name, size_t* length, uint16_t* padding) {
	...
	strncpy(name, input, strlen(input));
...
WARN_UNUSED_RESULT int modifyIpRule(uint16_t action, uint32_t priority, uint32_t table,
    ...
    rule.action = static_cast<uint8_t>(priority != RULE_PRIORITY_UNREACHABLE ? FR_ACT_TO_TBL :
                                                                               FR_ACT_UNREACHABLE);
...
int modifyVpnOutputToLocalRule(const char* vpnInterface, bool add) {
#if 0
...
#else
	ALOGW("modifyVpnOutputToLocalRule unimpled");
#endif
}
